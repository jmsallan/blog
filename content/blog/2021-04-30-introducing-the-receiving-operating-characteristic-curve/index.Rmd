---
title: Introducing the receiving operating characteristic curve
...
In [a previous post](https://jmsallan.netlify.app/blog/a-workflow-for-binary-classification-with-tidymodels/) I presented a small binary classification problem, consisting in finding if an observation of the `iris` dataset belongs to the versicolor species. In a binary classification problem, the target variable must be a factor, with first level associated to the positive case:
...
Here I will introduce the **receiving operating characteristic (ROC)** curve in the context of performance assessment of classification problems. We'll assess the performance of two predictive models with the ROC curve, and then examine the performance of a bad classifier with the same metric.
