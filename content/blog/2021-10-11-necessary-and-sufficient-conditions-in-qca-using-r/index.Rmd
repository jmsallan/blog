...
We observe that all elements of set `SURV` are also included in sets `DEV`, `LIT` and `STB`. So, these are necessary conditions for `SURV` to happen. The reverse implication does not hold: for instance, if we pick all observations where `DEV` occurs we observe that there are two observations not belonging to `SURV`.

```{r}
LC %>% select(DEV, SURV) %>% filter(DEV == 1)
```

With fuzzy sets, the condition of necessity that $Y$ is a subset of $X$ means that **if $X$ is a necessary condition of $Y$, fuzzy scores of $X$ are higher than fuzzy scores of $Y$.** If we plot fuzzy memberships of $Y$ versus fuzzy memberships of $X$, observations of necessary conditions will appear in the lower area of the plot:
...
Thirteen out of eighteen observations of the dataset are included in `LIT`. As a consequence, its relevance is of 0.5 and it can be considered as a condition of low relevance. .
...
Note that as scores of $X$ should be higher than scores of $Y$, most of the observations must be in the lower triangle for a necessary condition to be consistent. This is what is happening with `DEV` and `LIT`, but not with `URB`. In the `LIT` plot, we observe that many observations have an score near to one: that's why `LIT` has less relevance than `DEV`.
...
In terms of set theory, $X$ is a sufficient condition for $Y$ if $X$ is a subset of $Y$. When applying fuzzy logic, **the fuzzy scores of $Y$ must be higher than the ones of $X$**. For perfect sufficiency, all observations must be in the upper triangle of the $\left(X,Y\righ)$ plot. 
