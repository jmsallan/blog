...
Let's obtain the solution using `solve_lp`. Here I am creating a `model.mod` file with `writeLines` and deleting it after use with `unlink`.
...
Mathprog also allows writing not only linear programs as formulations of a generic problem that can be applied to different instances. Let's see the (abridged) formulation of the transportation problem:
...
The formulation of the transportation problem above uses sets to define the parameters and variables of the model. We can also use formulations with subindices, like in this formulation of a production planning with acquisition and storage costs:
...
```{r}
writeLines(prod_plan, "model.mod")
solve_lp("model.mod", type = "MathProg")
unlink("model.mod")
```
